...
def plot_confMat(target, predicted, out_parentDir="./confusion_matrix_result"):
    ...
    plt.savefig(f"{out_parentDir}/confusion_matrix_se_binary.png", bbox_inches="tight", pad_inches=0)
...
class CustomDataset(Dataset):
    def __init__(self, dataset, directory, transform=None, is_training = False):
        ...
        if (is_training):
            for file in self.dataset:
                ...
                image = np.concatenate((image_raw[np.newaxis, ...], image_seg[np.newaxis, ...]), axis=0)
                image_sum += np.mean(image, axis=(1, 2), keepdims=True)
                image_squared_sum += np.mean(image * image, axis=(1, 2), keepdims=True)
                count += 1
            # shape: (2, 1, 1)
    ...
    def __getitem__(self, idx):
        ...
        image = np.stack([image_raw, image_seg])
...
model = OAI_KLG_Model(in_channel=2, latent_channels=[8, 16, 64, 128, 256, 256], use_slice=False, is_binary_problem = True)
...
plt.savefig("./sen_bin_loss_graph.png")
